...
const DATA_TAG_OPEN = new RegExp(`<${DATA_TAG}>(.*?)$`);
const DATA_TAG_CLOSR = new RegExp(`^(.*?)</${DATA_TAG}>`);
...
const worker = (command, options = {}) => {
    ...
    let run = (resolve, reject) => {
        ...
        child.stdout.on('data', content => {
            ...
            if (cache || DATA_TAG_OPEN.test(content)) {
                cache += content;
                content = content.replace(DATA_TAG_OPEN, '');
                content = content.replace(DATA_TAG_CLOSR, '')
            }
